...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if len(verification_idx) > 0:
            # NOTE: add verification prompt to the end of the response (sicer)
            verification_text = "Is the answer correct (Yes/No)?"
            tokens = tokenizer.encode(verification_text)
            # response = [res + tuple(tokens) for res in response]

            # add a generation process
            verification_idx_list = [idx for idx in idx_list for _ in range(n_samples)]
